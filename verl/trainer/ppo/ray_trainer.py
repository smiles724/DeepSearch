...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, multi_turn=False, norm_adv_by_std_in_grpo=True, **kwargs):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        enabled_mcts = kwargs.get("enabled_mcts", False)
        mcts_config = kwargs.get("mcts_config", {})
        use_step_wise_score = mcts_config.get("use_step_wise_score", True)
        if enabled_mcts and use_step_wise_score:
            from deepsearch.reward_manager.advantage_fn import compute_mcts_outcome_advantage
            print(
                f"[compute_advantage] Enter MCTS Advantage Function with token_level_rewards "
                f"shape={data.batch['token_level_rewards'].shape} and mcts_config={mcts_config} norm_adv_by_std_in_grpo={norm_adv_by_std_in_grpo}!"
            )
            advantages, returns = compute_mcts_outcome_advantage(
                token_level_rewards=data.batch["token_level_rewards"],
                response_mask=grpo_calculation_mask,
                mcts_config=mcts_config,
                index=data.non_tensor_batch.get("uid"),  # Pass index for sibling normalization
                norm_adv_by_std_in_grpo=norm_adv_by_std_in_grpo,
            )

        else:
            print(
                f"[compute_advantage] Enter GRPO Advantage Function with token_level_rewards "
                f"shape={data.batch['token_level_rewards'].shape} and mcts_config={mcts_config}!"
            )
...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            if "mcts_config" in self.config.actor_rollout_ref.rollout:
                # Create uid before repeating for MCTS
            ...
            if "mcts_config" in self.config.actor_rollout_ref.rollout:
                ...
                test_gen_batch.non_tensor_batch["index"] = test_batch.non_tensor_batch["index"]
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # TODO: check this. add uid and reward_model into gen_batch for MCTS rollout
                if "mcts_config" in self.config.actor_rollout_ref.rollout:
                    # Create uid before popping gen_batch (for MCTS), uid here is different from the ones below for batch
                ...
                with _timer("step", timing_raw):
                    ...
                    if "mcts_config" in self.config.actor_rollout_ref.rollout:
                        # pop out the uids used for MCTS searching
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        batch = compute_advantage(
                            ...
                            enabled_mcts=hasattr(self.config.actor_rollout_ref.rollout, "mcts_config"),
                            mcts_config=self.config.actor_rollout_ref.rollout.get("mcts_config", {}),
                        )
