    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        if rollout_name == "hf":
            ...
            if vllm_mode == "customized":
                ...
                if "mcts_config" in self.config.rollout:
                    # TODO: Implement DeepSearch wrapper for the vLLM engine
            ...
            if "mcts_config" in self.config.rollout:
                from deepsearch.rollout.sglang_mcts_wrapper import DeepSearchSGLangRollout

                sglang_rollout_cls = SGLangRollout
                ...
                rollout = DeepSearchSGLangRollout(
                    sglang_rollout_cls=sglang_rollout_cls,
                    sglang_rollout_kwargs=sglang_rollout_kwargs,
                    tokenizer=self.tokenizer,
                    mcts_config=self.config.rollout.mcts_config,
                )
                logger.info(f"Successfully Initialize DeepSearchSGLangRollout by mcts_config={self.config.rollout.mcts_config}")
            else:
