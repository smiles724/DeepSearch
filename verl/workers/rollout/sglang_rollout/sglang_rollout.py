...
class SGLangRollout(BaseRollout):
    ...
    def _init_inference_engine(self, trust_remote_code, actor_module, port):
        ...
        if first_rank_in_node:
            ...
            engine_kwargs = {}
            if hasattr(self.config, "engine_kwargs") and hasattr(self.config.engine_kwargs, "sglang"):
                engine_kwargs.update(**self.config.engine_kwargs.sglang)

            import socket
            import threading
            # Get more unique identification to avoid log deduplication
            hostname = socket.gethostname()
            process_id = os.getpid()
            thread_id = threading.get_ident()

            print(
                f"🚀 [WORKER-{rank}@{hostname}:{process_id}:{thread_id}] SGLang VerlEngine initialization: "
                f"tp_size={self._tp_size}, tp_rank={self._tp_rank}, node_rank={node_rank}, rank={rank}, "
                f"first_rank_in_node={first_rank_in_node}, tp_size_per_node={tp_size_per_node}, "
                f"dist_init_addr={dist_init_addr}, nnodes={nnodes}, engine_kwargs={engine_kwargs}"
            )

            # Additional distributed state logging
            logger.warning(f"📊 [WORKER-{rank}@{hostname}:{process_id}] Distributed state check - "
                           f"CUDA_VISIBLE_DEVICES={os.environ.get('CUDA_VISIBLE_DEVICES', 'Not Set')}, "
                           f"LOCAL_RANK={os.environ.get('LOCAL_RANK', 'Not Set')}, "
                           f"WORLD_SIZE={os.environ.get('WORLD_SIZE', 'Not Set')}")
